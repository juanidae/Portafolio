...
def export_to_excel(fact: pd.DataFrame, dim_sprint: pd.DataFrame,
                    ...
                    fact_issueschangelog: pd.DataFrame,
                    dim_sprint_changelog: pd.DataFrame,
                    dim_sprint_all: pd.DataFrame,
                    fact_changelog_all: pd.DataFrame,
...
def main():
    ...
    fact_changelog_all   = filter_fact_by_sprints_changelog(
        build_fact_issueschangelog(all_issues_changelog),
        dim_sprint_all
    )
    ...
    df_control = pd.DataFrame({
        ...
        "RegistrosFact": [len(fact)],
        "RegistrosChangelog": [len(fact_issueschangelog)]
    })
